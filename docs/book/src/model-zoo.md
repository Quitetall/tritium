...
|----------------|--------|------------------------|
| **BitNet b1.58 2B4T**, `I2_S` GGUF, **CPU** backend | **Verified-compatible** | `crates/tritium-nn/tests/fidelity_ladder.rs` (stage-by-stage CPU forward vs a `transformers` oracle) and `acceptance.rs::cpu_longer_greedy_matches_transformers` (greedy IDs vs the committed reference) — both **model-gated** (they self-skip when the GGUF is absent). |
