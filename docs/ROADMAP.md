...
| 0050 | `docs/plans/0050-web-training-session.md` | Strict-TypeScript compiled training session, deterministic WASM fallback, complete-manifest WebGPU, real-browser receipts and local npm archive | v1.1 / ADR 0033; plan 0049 frozen schema | **in progress** — reproducible strict-TypeScript local RC, checked lifecycle, immutable operation/buffer planning, tied-parameter allocation, compiled one-gradient/one-optimizer ownership, exact batch staging, and an archive-bundled actual `wasm32-unknown-unknown` 35-op/114-case deterministic conformance guest complete; WASM lifecycle controller, accumulation execution, WebGPU and browser receipts remain open | WASM request/controller ABI → tied-gradient execution → WebGPU → browser receipts → archive/tutorial |
