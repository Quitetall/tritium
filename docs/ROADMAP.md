...
| 0037 | `docs/plans/0037-qwen-arch.md` | Qwen-family arch: **QKV-bias** (Qwen2/2.5) + **QK-norm** (Qwen3) + explicit **head_dim** (decoupled from n_embd/n_head). Detected from weights, applied per-head before RoPE. Gate: **Qwen2.5-0.5B 16/16 + Qwen3-0.6B 16/16** token-exact vs transformers (rel-err <5e-6); SmolLM2 still 16/16 | ADR 0020 (arch) | **✅ DONE** — reaches the 32B/Qwen targets | — |
