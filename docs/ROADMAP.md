...
| 0050 | `docs/plans/0050-web-training-session.md` | Strict-TypeScript compiled training session, deterministic WASM fallback, complete-manifest WebGPU, real-browser receipts and local npm archive | v1.1 / ADR 0033; plan 0049 frozen schema | **in progress** — reproducible strict-TypeScript local RC, checked lifecycle, immutable operation/buffer planning, tied-parameter allocation, compiled reverse VJP/fan-in schedule, exact batch staging, archive-bundled actual `wasm32-unknown-unknown` 35-op/114-case deterministic conformance, strict digest-bound portable request execution, generated 31-op/57-execution schedule bindings, canonical checksummed initial tensor payload, pre-guest structural/subset and portable-capacity admission, stable adapter errors, session-owned WASM forward/backward/atomic optimizer execution, canonical checkpoint/resume and planner-accounted phase-peak receipts complete; operation-specific geometry admission, state-derived export, WebGPU and physical browser receipts remain open | geometry admission → state-derived export → WebGPU → browser receipts → archive/tutorial |
