...
| v0.80 Interop | 0010 | In progress — **`tritium-serve` done** (OpenAI HTTP/SSE, `v0.6.2`; doubles as the LAMU `backend_kind`); **`tritium-ffi` done** (C ABI cdylib + staticlib, panic-safe, cbindgen header w/ drift + C11/C++17 gate, `v0.6.3` — unblocks the v1.0 C-ABI freeze); **`tritium-candle` done** (ternary mpGEMM as a candle `CustomOp1`, bit-exact conformance gate, `v0.6.4`); `burn`/`onnx` remain |
...
→ **`tritium-ffi` (C ABI, done `v0.6.3`)** → **`tritium-candle` (candle CustomOp, done `v0.6.4`)** + burn / onnx (rest of v0.80, pure-CPU wrappers over the shipped runner) →
