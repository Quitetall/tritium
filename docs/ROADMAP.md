...
| v0.60 Pretraining + Distributed | 0008 | **In-progress** — full-model CPU backward (0011, `v0.5.1`) + data pipeline (0012, `v0.5.2`) + GPU pretrain smoke (0013, `v0.5.3`) + ProcessGroup trait & simulated collective backend (0014, `v0.5.4`) + ZeRO-3/FSDP over the sim PG with gradient + loss parity (0015, `v0.5.5`) + distributed checkpoint with resharding + crash-atomic writes (0016, `v0.5.6`) **done**; the entire single-GPU-reachable foundation (0011–0016) is shipped on the `0.5.x` line. **Remaining: only the real ≥2-GPU wall** (0017 NCCL backend + 0018 HW loss-parity/≥80% scaling), deferred to one rented 2×GPU session → `v0.60.0` |
