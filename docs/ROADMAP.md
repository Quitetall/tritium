...
| 0050 | `docs/plans/0050-web-training-session.md` | Strict-TypeScript compiled training session, deterministic WASM fallback, complete-manifest WebGPU, real-browser receipts and local npm archive | v1.1 / ADR 0033; plan 0049 frozen schema | **in progress** — reproducible strict-TypeScript local RC, checked lifecycle, immutable operation/buffer planning, tied-parameter allocation and exact batch staging complete; WASM controller, tied-gradient accumulation, WebGPU and browser receipts remain open | WASM controller → tied gradients → WebGPU → browser receipts → archive/tutorial |
