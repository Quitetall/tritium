...
| 0049 | `docs/plans/0049-portable-training-manifest.md` | Frozen language-neutral training operation registry, semantic vectors, fallible backend seam, and native/constrained backend receipts | v1.1 / ADR 0033; plan 0044 | **in progress** — work order, Rust registry/audit, and strict byte-identical Python/TypeScript parsers landed; semantic vectors and backend adapters remain open | vectors → CPU seam → CUDA/ROCm/Metal/wgpu → WASI/MCU → receipts |
| 0050 | `docs/plans/0050-web-training-session.md` | Strict-TypeScript compiled training session, deterministic WASM fallback, complete-manifest WebGPU, real-browser receipts and local npm archive | v1.1 / ADR 0033; plan 0049 frozen schema | **in progress** — work order frozen; package and executor implementation open | package/schema → WASM controller → session → WebGPU → browser receipts → archive/tutorial |
