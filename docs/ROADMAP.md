...
| 0043 | `docs/plans/0043-salt-v2-sota-campaign.md` | **ACTIVE CAPSTONE — SALT V2 on pinned Qwen3.6-27B.** Ternarize the language core plus bundled one-layer MTP drafter; identity-bind deferred vision tensors; run PTQ and refined tracks as separate artifacts; retain exact physical-rate, semantic D2/B3/S34, native-runtime, quality, provenance, and publication gates | ADR 0028 + amended ADR 0020; consumes ADR 0026 claim gates | **in progress** (software/reference substrate advancing; Qwen3.6 adapter, dense parity, campaign driver, and empirical gates open; no paid run authorized) | Qwen3.6 adapter/parity → identity+coverage+accounting → local gates → 27B PTQ → refined → baselines/evidence |
