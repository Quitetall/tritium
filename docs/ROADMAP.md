...
| **v1.x Capstone — additive ternarization of a SOTA model (BINDING public-launch gate)** | **0020 / 0028** | **Active target selected; empirical gates open.** The binding checkpoint is [`Qwen/Qwen3.6-27B`](https://huggingface.co/Qwen/Qwen3.6-27B/tree/6a9e13bd6fc8f0983b9b99948120bc37f49c13e9), revision `6a9e13bd6fc8f0983b9b99948120bc37f49c13e9`. Convert the language core plus bundled one-layer MTP drafter; defer vision/multimodal integration. PTQ and refined tracks remain separately identified. The inherited ≤1% perplexity delta, quality-vs-physical-bpw curve, end-to-end Tritium execution, and measured memory-reduction gates remain binding. A standard 32B is optional only. Local-first; no paid compute is authorized. Active execution: plan 0043. |
| **v1.1 Full Public Release — ternary research platform** | **0033** | **Accepted; implementation in progress.** PyTorch/Hugging Face phased differentiable workflow, SOTA estimator/PTQ/refinement catalog, whole-Tape semantic parity across every inference backend, compiled TypeScript/WebGPU training, full packaging/deployment/community surface, three-tier audited model zoo, and plan-0043 flagship proof. v1.2 binds native Rust plus non-PyTorch Python; v1.3 binds trainable ONNX. Active execution: plan 0044. |
...
| — | `docs/plans/capstone-cascade.md` | **Legacy milestone map** for the superseded standard-32B-first ADR 0020 cascade (0035→0041). Retained for history and reusable completed work; plan 0043 is the active execution order | v1.x capstone / ADR 0020 | **historical** (superseded 2026-07-15) | — |
| 0036 | `docs/plans/0036-salt-inference.md` | SALT-quantized model loader — `load_salt`/`from_salt` run what `quantize` emits via **dequant-to-dense** (bit-equal to native multi-plane; norms+config from the model dir). `salt_rows_to_dense`, shared `build_standard_model`. Gate: quantize→bundle→`from_salt` wires the dequant + runs (finite logits). GPU-native multi-plane (VRAM win) → 0040 | ADR 0020 step 2 | **✅ DONE** — CPU dequant path; SmolLM2 conformance still 16/16 | — |
| 0037 | `docs/plans/0037-qwen-arch.md` | Qwen-family arch: **QKV-bias** (Qwen2/2.5) + **QK-norm** (Qwen3) + explicit **head_dim** (decoupled from n_embd/n_head). Detected from weights, applied per-head before RoPE. Gate: **Qwen2.5-0.5B 16/16 + Qwen3-0.6B 16/16** token-exact vs transformers (rel-err <5e-6); SmolLM2 still 16/16 | ADR 0020 (arch) | **✅ DONE** — conventional Qwen2.5/Qwen3 foundation; the Qwen3.6 hybrid+MTP adapter remains open in 0043 | — |
| 0038 | `docs/plans/0038-salt-distillation.md` | **SALT-aware distillation trainer** — latent-master QAT, SALT-as-STE. **Steps 1–4 DONE:** multi-plane SALT-STE (`salt_ste`) — atomic recovery **92.6%** of the PTQ gap; **SiLU** op (gradchecked) for a SwiGLU student; **end-to-end distillation of a tiny SwiGLU transformer** (attention+SwiGLU+lm_head, softmax_xent=KL gradient) — student KL to the fp teacher drops (xent 1.87→1.77). Next (step 5): teacher-logit cache + real-model ppl recovery | ADR 0020 step 3 | **in-progress** (4/5) | — |
| 0039 | _(cascade)_ | Legacy Fisher/adaptive-growth stage; its representation-independent curvature and allocation work is absorbed into the separately labeled PTQ/refined tracks in plan 0043 | ADR 0020 historical step 4; superseded by ADR 0028 / plan 0043 | **superseded** (use 0043 gates) | — |
| 0040 | _(cascade)_ | Legacy standard-32B scale-plumbing stage; streaming conversion, resident load, checkpointing, and offload requirements transfer to pinned Qwen3.6-27B without authorizing the standard-32B run | ADR 0020 historical step 5; superseded by ADR 0028 / plan 0043 | **superseded** (requirements absorbed by 0043) | — |
| 0041 | _(cascade)_ | **Legacy standard-32B capstone.** Its ≤1% ppl, quality-vs-bpw, measured-memory, and end-to-end evidence requirements are inherited by the active Qwen3.6 campaign, but the standard 32B run itself is optional and no longer precedes the flagship | ADR 0020 historical step 6; superseded by ADR 0028 / plan 0043 | **superseded** (optional confirmation only) | — |
| ADR 0026 | `docs/adr/0026-sota-campaign.md` | **SOTA campaign** — 4 tracks to close every measured gap (P: IMMA prefill ≥6K tok/s; D: verify graph ≤4.5ms + BLUT drafter ≥2×; Q: Bonsai ingest+eval + SALT-vs-PT²-LLM; R: `report compare` + BENCHMARKS.md ledger); exit = best measured ternary number in every regime, reproducible from one command | consumes SOTA head-to-head (`research-ternary-sota-mid2026.md` §7); sequences ADR 0021/0024, plans 0037–0042 | **proposed** (2026-07-11) | P ∥ D1 ∥ Q1 |
| 0043 | `docs/plans/0043-salt-v2-sota-campaign.md` | **ACTIVE CAPSTONE — SALT V2 on pinned Qwen3.6-27B.** Ternarize the language core plus bundled one-layer MTP drafter; identity-bind deferred vision tensors; run PTQ and refined tracks as separate artifacts; retain exact physical-rate, semantic D2/B3/S34, native-runtime, quality, provenance, and publication gates | ADR 0028 + amended ADR 0020; consumes ADR 0026 claim gates | **in progress** (source-bound Qwen adapter, fixture-only MTP parity, durable factor evidence, bounded tensor fitter, resumable Rust/Python pure-PTQ master driver, strict schema-v3 packed CPU/CUDA runtime, fresh-wheel Python generation, and RTX 4090 resident-allocation fixture proof landed; pinned-27B evidence collection/run, checkpoint-scale 64-layer CUDA/performance proof, MTP production promotion, refinement, and empirical gates remain open; no paid run authorized) | evidence collection → pinned-27B adapter/parity → local gates → 27B PTQ → refined → baselines/evidence |
| 0044 | `docs/plans/0044-v11-full-public-release.md` | **ACTIVE RELEASE WORK ORDER — Tritium v1.1 full public platform.** Cross-language schemas; PyTorch/HF phased autograd/PTQ/refinement; whole-Tape backend parity; compiled browser training; ONNX inference; packages; production deployment/observability; governance/docs; audited zoo; release reproduction | ADR 0033; parallel dependency on plan 0043 | **in progress** (0045 done; 0046–0048 active; 0049 portable parity, 0050 browser and 0051 ONNX/package work orders active; 0052–0053 remain reserved) | 0046 Torch ops → parallel 0047/0048 → 0049 schema/portable parity → 0050–0052 → 0053 release |
...
| 0047 | `docs/plans/0047-huggingface-qat-ptq-facade.md` | Hugging Face QAT/Trainer/PTQ/export/reload facade | v1.1 / ADR 0033 | **in progress** — QAT/HF state, expert Qwen PTQ-master binding, exact physical nested allocation, packaged schema-v3 `QuantizationResult`, atomic save/reload, strict packed CPU/CUDA runtime, fresh-wheel smoke, and Python token generation landed; raw activation collection, pinned-27B/MTP production proof, accelerator FSDP, and ordinary HF generation closure remain open | 0046 semantic gate |
| 0048 | `docs/plans/0048-estimator-catalog-salt-refinement.md` | Differentiable estimator/plugin catalog plus production SALT reconstruction and separately typed refinement tracks | v1.1 / ADR 0033; plan 0043 | **in progress** — estimator catalog landed; production reconstruction/refinement open | 0045 reference types + 0043 driver seams; final schemas close in 0049 |
| 0049 | `docs/plans/0049-portable-training-manifest.md` | Frozen language-neutral training operation registry, semantic vectors, fallible backend seam, and native/constrained backend receipts | v1.1 / ADR 0033; plan 0044 | **in progress** — registry and 114-case corpus complete; CPU, actual wasmtime, actual RTX 4090 CUDA, and actual RTX 4090 native wgpu each cover all 35 operations and 114 vectors; canonical receipt sealing/admission and receipt-generated tables are implemented and CPU-proven; ROCm, Metal, MCU, and candidate-revision physical receipt artifacts remain open | ROCm/Metal → MCU → candidate receipts |
| 0050 | `docs/plans/0050-web-training-session.md` | Strict-TypeScript compiled training session, deterministic WASM fallback, complete-manifest WebGPU, real-browser receipts and local npm archive | v1.1 / ADR 0033; plan 0049 frozen schema | **in progress** — reproducible strict-TypeScript local RC, checked lifecycle, immutable operation/buffer planning, tied-parameter allocation, compiled reverse VJP/fan-in schedule, exact batch staging, archive-bundled actual `wasm32-unknown-unknown` 35-op/114-case deterministic conformance, strict digest-bound portable request execution, generated 31-op/57-execution schedule bindings, complete allocation-free 31-operation ABI/geometry/attribute/scratch admission, canonical checksummed initial tensor payload, pre-guest structural/subset and portable-capacity admission, stable adapter errors, session-owned WASM forward/backward/atomic optimizer execution, canonical checkpoint/resume, planner-accounted phase-peak receipts, live-state-derived compact B3 SALT V2 export with strict WASM reload, typed transactional cancellation/device-loss failure receipts, a SHA-256-bound archive-bundled WGSL candidate set for all 31 tensor operations, a shared 57-form WebGPU dispatch catalog with source-derived bindings/workgroups plus physical pipeline compilation and 114-vector RTX 4090 conformance, a limit-admitted resident WebGPU runtime with owner and auxiliary constant/scratch buffers, checked same-submission GPU copies, aligned uniforms, cached bindings, single-command-buffer submission and explicit-only readback, and plan-level resident lowering for 46/57 forms including pointwise, SALT, FSQ, embedding, RoPE, concat and softmax cross entropy complete; convolution/attention lowering, optimizer candidate commits, session adapter integration, physical loss injection, and browser receipts remain open | Remaining specialized WebGPU lowering/optimizer commits/session adapter → physical loss/browser receipts → archive/tutorial |
| 0051 | `docs/plans/0051-onnx-packaging-colab.md` | Whole-model packed ONNX inference, clean crate/wheel/npm RCs, compatibility/SBOM/provenance, and five-minute Colab | v1.1 / ADR 0033; plans 0046/0047/0049 | **in progress** — work order frozen; implementation open | ORT graph → HF facade → local RC → compatibility → Colab → authorized activation |
