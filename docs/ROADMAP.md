...
| v0.70 Backend Breadth | 0009 | **Code-complete** — wgpu + wasm validated (`v0.6.1`); **`tritium-metal` (MSL) + `tritium-rocm` (HIP) written + Linux-green-gated (`v0.6.8`)**, with self-hosted `metal`/`rocm` CI lanes. The full backend matrix (cpu/cuda/wgpu/wasm/metal/rocm) now exists. **Milestone tag gated** on Metal + ROCm parity validation on real Apple-Silicon / AMD hardware (the GPU code's first compile + the frozen-vector conformance run). |
