...
| v0.70 Backend Breadth | 0009 | **Code-complete; Metal parity ✅ validated on real hardware.** wgpu + wasm validated (`v0.6.1`); `tritium-metal` (MSL) + `tritium-rocm` (HIP) written (`v0.6.8`). **Metal first-compiled + passed the 89-vector conformance bit-exact on a real Apple M1** (Scaleway M1-M, macOS 26.3.2, `v0.6.9`, zero glue fixes) — and wgpu re-validated there via its Metal HAL; the Metal backend now keeps TQ2_0 packed on-device (in-kernel decode, memory-parity with cuda/rocm). **Milestone tag gated** on the remaining **ROCm parity** validation on real AMD hardware (its first compile + frozen-vector conformance run). |
| v0.80 Interop | 0010 | **Done** (all four framework backends on the 0.6.x line) — **`tritium-serve`** (OpenAI HTTP/SSE, `v0.6.2`; LAMU `backend_kind`); **`tritium-ffi`** (C ABI cdylib + staticlib, panic-safe, cbindgen header w/ drift + C11/C++17 gate, `v0.6.3` — unblocks the v1.0 C-ABI freeze); **`tritium-candle`** (candle `CustomOp1`, bit-exact, `v0.6.4`); **`tritium-burn`** (backend-generic burn op, bit-exact, `v0.6.5`) + **`tritium-onnx`** (always-on bit-exact kernel + `ort` 2.x custom op, `v0.6.5`) |
| v0.90 Hardening | 0011 | In progress — reachable tooling **done** (`v0.6.6`): mdbook guide + dead-link `docs` CI lane; CPU `sanitizers` lane (ASan/MSan/TSan + miri, clean); `wheels` lane (abi3 maturin, artifacts); `cpu-bench-smoke` lane. Earlier: cargo-deny (`v0.5.8`), fuzz breadth (`v0.5.9`), doc-coverage + semver baseline (`v0.5.10`), publish-check + SBOM + SECURITY (`v0.6.x`). **Fenced-HW remainder:** Metal/ROCm platform-GPU lanes + the full GPU CI matrix. |
| v1.0 Release | 0012 | In progress — reachable prep **done** (`v0.6.7`): API/C-ABI freeze-readiness audit (`docs/v1.0-api-freeze-audit.md`, C ABI frozen at v1, semver gate green vs `v0.5.10`); model-zoo + benchmark mdbook chapters; CPU fresh-env `capstone` e2e (install→infer→SALT→fine-tune, real code paths) + CI lane. **v1.0.0 tag gated** on fenced HW: Metal/ROCm parity, the GPU CI matrix, and the real-model/GPU capstone (download BitNet 2B4T + GPU fine-tune recovering accuracy). |
