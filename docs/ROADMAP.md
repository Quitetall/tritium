...
| 0043 | `docs/plans/0043-salt-v2-sota-campaign.md` | **ACTIVE CAPSTONE — SALT V2 on pinned Qwen3.6-27B.** Ternarize the language core plus bundled one-layer MTP drafter; identity-bind deferred vision tensors; run PTQ and refined tracks as separate artifacts; retain exact physical-rate, semantic D2/B3/S34, native-runtime, quality, provenance, and publication gates | ADR 0028 + amended ADR 0020; consumes ADR 0026 claim gates | **in progress** (source-bound Qwen family adapter and fixture-only MTP parity gate landed; pinned-27B 64-layer/coverage/CUDA proof, campaign driver, and empirical gates remain open; no paid run authorized) | pinned-27B adapter/parity → identity+coverage+accounting → local gates → 27B PTQ → refined → baselines/evidence |
...
| 0045 | `docs/plans/0045-torch-reference-conversion.md` | Pure-PyTorch reference seam: typed config/errors/coverage/projection, `TernaryLinear`, transactional recursive `prepare_qat`, tied-weight/state preservation and exact parameter accounting | ADR 0033 / plan 0044 | **in progress** | CPU Python tests; blocks 0046 and 0047 |
