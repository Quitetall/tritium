...
| 0038 | `docs/plans/0038-salt-distillation.md` | **SALT-aware distillation trainer** — latent-master QAT, SALT-as-STE (reuses Tape/AdamW/ste from tritium-train; softmax_xent = correct KL gradient). **Step 1 DONE:** multi-plane SALT-STE op (`salt_quantize_forward`/`salt_ste`) — distillation **recovers 92.6% of the PTQ gap** on the atomic gate. Next: silu op, layerwise projection heal, model ppl-recovery, e2e logit-KL | ADR 0020 step 3 | **in-progress** (step 1/5) | — |
