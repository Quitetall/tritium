...
| v0.60 Pretraining + Distributed | 0008 | **In-progress** — full-model CPU backward (0011) **done** (tagged `v0.5.1`); shipping the single-GPU-reachable foundation as the `0.5.x` line (data pipeline, GPU training loop + pretrain smoke, distributed correctness via a simulated ProcessGroup), with the real ≥2-GPU wall (NCCL + ≥80% scaling) deferred to a rented session → `v0.60.0` |
...
| 0012+ | (planner writes just-in-time) | Data pipeline (0012), GPU training loop + pretrain smoke (0013), ProcessGroup + ZeRO-3 sim + resharding (0014–0016), then the ≥2-GPU wall (0017–0018) | v0.60 / ADR 0008 | todo | per chunk |
