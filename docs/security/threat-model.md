...
| Floating toolchain and unpinned tool installs in CI | Tampering | low | `rust-toolchain.toml` pins the BUILD toolchain to an exact Rust version (`1.98.0`) rather than a floating channel; GitHub Actions are SHA-pinned; `rust-version="1.96"` is the published compatibility floor (policy: latest stable minus two, SUPPORT.md), now decoupled from the build pin and verified by the `msrv` CI lane rather than assumed; SBOM generated + uploaded as artifact (ci.yml:325-337); cargo-deny-action pins a recent cargo-deny deliberately. | External tool installers and SBOM output are not baseline-diffed; trusted action/tool inputs can still drift where their own versions are not frozen. Low impact for a library; weakens reproducibility/audit trail. |
