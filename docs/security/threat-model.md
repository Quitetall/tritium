...
| Threat | STRIDE | Severity | Mitigation (cited) | Residual risk |
|---|---|---|---|---|
| No authentication or authorization on any endpoint | Elevation of Privilege | low | Optional bearer-token auth (`auth_token` in `ServeConfig`) is enforced by middleware over the whole surface, including `/healthz`, `/readyz`, and the 404 fallback (uniform — no unauthenticated route probing); constant-time token compare; 401 carries `WWW-Authenticate: Bearer`; non-loopback bind requires the token; contract tests pin missing/wrong/right-token behavior. | Loopback default runs unauthenticated unless the operator sets a token — on shared hosts any local user qualifies. Length of the token leaks via the compare's length short-circuit; marginal. Authenticated probes need a static header until plan 0052's separate admin listener lands. |
| No request timeout — slowloris / slow-send | DoS | low | Explicit 2 MiB `DefaultBodyLimit`; `tower_http::TimeoutLayer` (default 600 s) bounds the non-streaming service future; the lazy SSE body independently enforces the same absolute lifetime from queue admission, emits an OpenAI `request_timeout` error event, and drops its receiver to cancel generation; `ConcurrencyLimitLayer` (default 64) caps in-flight requests; generation uses a bounded queue. | Permit wait at the concurrency limiter is outside the timeout clock (mitigated by queue 429 backpressure). The non-streaming 408 generated by tower-http has an empty body rather than the OpenAI error shape. |
| Unbounded prompt length & `max_tokens` value (compute exhaustion within 2MB body) | DoS | low | `RequestLimits` rejects excessive message count, aggregate role/content bytes, tokenized prompt length, explicit completion length and checked combined token budget before queue admission; the 2 MiB body limit bounds JSON parsing; excessive `max_tokens` is rejected instead of silently clamped; bounded queue returns 429. | Tokenization still occurs before the token-count limit can be applied. Single decode thread implies head-of-line blocking, bounded by queue and request lifetime. Per-principal rate limiting remains plan 0052 work. |
| Streaming (SSE) connection exhaustion via many concurrent streams | DoS | low | Bounded job queue returns 429 + Retry-After; per-token `try_send` cancels stalled readers; global `ConcurrencyLimitLayer` (default 64) caps concurrent in-flight requests; the absolute SSE lifetime cancels a stream even when tokens continue flowing; graceful drain closes in-flight streams — the batched worker also errors queued-but-unadmitted jobs before prefill. | 64-token-per-job buffer times active streams is bounded. An active SSE stream holds a concurrency permit until completion or deadline; deployment sizing and per-principal admission remain required for hostile multi-tenant exposure. |
...
7. ~~**No request timeout / connection cap.**~~ **DONE** — service and lazy-SSE deadlines plus a global in-flight cap are enforced; the non-streaming timeout body-shape residual is tracked above.
