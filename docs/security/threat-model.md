...
| `compute_zero_bitmap` / `compute_zero_bitmaps` panic or OOB-slice on under-length / overflowing input | DoS | info | Both return `FormatError::WrongBlockLen` on under-length input (tq2.rs:97-124); `compute_zero_bitmaps` additionally guards `n * row_bytes` with `checked_mul` so a wrapped product cannot pass the length check then slice out of bounds (tq2.rs:133-148); fuzz target `zero_bitmap` sweeps (n, k, row_bytes) including absurd values (fuzz_targets/zero_bitmap.rs, scheduled CI lane). | Closed. Callers get a typed error, never a panic; fuzzed alongside every other untrusted-byte entry point (`unpack_i2s` / `unpack_tq_rows` targets cover the row decoders too). |
...
Binds **loopback by default** and is **off by default** (`serve` not in default features; bin `required-features=["serve"]`). An explicit `--host <ip>` flag can bind beyond loopback, but a non-loopback bind **refuses to start unless `TRITIUM_AUTH_TOKEN` is set** (main.rs:98-118) and prints a loud exposure warning. Severities below are calibrated to the default localhost posture; the hardening layers (auth, timeout, concurrency cap, body limit) are what keep them bounded under exposure.

| Threat | STRIDE | Severity | Mitigation (cited) | Residual risk |
|---|---|---|---|---|
| No authentication or authorization on any endpoint | Elevation of Privilege | low | Optional bearer-token auth (`auth_token` in `ServeConfig`, router.rs:49) enforced by middleware over the WHOLE surface including `/healthz` and the 404 fallback (uniform — no unauthenticated route probing); constant-time token compare; 401 carries `WWW-Authenticate: Bearer`; non-loopback bind REQUIRES the token (main.rs:98-118); contract tests pin 401/401/200 behavior. | Loopback default runs unauthenticated unless the operator sets a token — on shared hosts any local user qualifies. Length of the token leaks via the compare's length short-circuit (same posture as `subtle`'s slice `ct_eq`); marginal. `/healthz`-behind-auth means LB health checks need a static header. |
| No request timeout — slowloris / slow-send | DoS | low | Explicit 2 MiB `DefaultBodyLimit` (router.rs:166, tested: 413); `tower_http::TimeoutLayer` (default 600 s, router.rs:170) bounds the service future — for NON-STREAMING requests that covers queue wait + the whole generation; `ConcurrencyLimitLayer` (default 64, router.rs:178) caps in-flight requests; generation decoupled onto a bounded queue (429 when full). | The timeout does NOT bound streaming requests: the SSE body is lazy, so the service future resolves at headers in microseconds — streaming slow-clients are instead bounded by the 64-event channel + per-token `try_send` cancellation. Permit wait at the concurrency limiter is outside the timeout clock (mitigated by queue 429 backpressure). The 408 from tower-http has an empty body, not the OpenAI error shape. |
...
| Streaming (SSE) connection exhaustion via many concurrent streams | DoS | low | Bounded job queue (cap 32) → 429 + Retry-After; per-token `try_send` cancels stalled readers (worker.rs:9-13,86-90); global `ConcurrencyLimitLayer` (default 64) caps concurrent in-flight requests; graceful-drain flag closes in-flight streams on shutdown — the batched worker also errors queued-but-unadmitted jobs before paying their prefill (batch.rs:170-190). | 64-token-per-job buffer × active streams is small/bounded. An SSE stream holds its concurrency permit for its whole life, so long streams can occupy the limit — sized for a single-GPU local server. |
