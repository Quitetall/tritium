...
The Slice 2 tracer corpus now carries 38 cases across 15 of 35 operations. In
addition to the primitive and shape clusters, it covers `graph.dense_matmul`
and scale-bearing `graph.ternary_matmul` forward/VJP semantics, plus `graph.transpose`, repeated
...
This is incremental evidence, not Slice 2 closure: the remaining 20 operations
