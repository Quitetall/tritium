...
| `packages` | PARTIAL | `compatibility-matrix` | **Not blocked — CI produces this on every release and it has never been harvested.** The rc.2 `abi3-compatibility-receipt` passes `aggregate-wheel-smoke.py`'s own validator: `tritium.abi3-matrix-qualification.v1`, bound to `d16c0dda`, `passed: true`, 16 cells spanning CPython 3.9.25–3.14.7 across three platforms and three distinct wheels. Harvesting it advances the union 15→16. It does **not** by itself close the gate: the release workflow now emits crate and npm archives plus qualification receipts; all four package kinds still must bind one exact revision before a coherent `packages` PASS. |
