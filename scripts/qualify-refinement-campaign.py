#!/usr/bin/env python3
"""Aggregate raw checkpoint refinement execution into release evidence."""
...
SCHEMA = VERIFIER["REFINEMENT_SCHEMA"]
TRACE_FIELDS = VERIFIER["REFINEMENT_TRACE_FIELDS"]
TRACE_SCHEMA = VERIFIER["REFINEMENT_TRACE_SCHEMA"]
canonical = VERIFIER["canonical"]
sha256 = VERIFIER["sha256"]
inventory = VERIFIER["inventory"]
object_ = VERIFIER["object_"]
derive_trace = VERIFIER["derive_refinement_trace"]
validate_receipt = VERIFIER["validate_refinement"]

MAX_TRACE_BYTES = 32 * 1024 * 1024


class QualificationError(ValueError):
    """Raw refinement execution cannot satisfy frozen admission."""
...
def require_clean_revision(repo: Path, revision: str) -> None:
    if git_output(repo, "rev-parse", "HEAD") != revision:
        raise QualificationError("refinement qualification source revision is not HEAD")
    if git_output(repo, "status", "--short", "--untracked-files=no"):
        raise QualificationError("refinement qualification requires clean tracked source")
...
def load_trace(path: Path) -> dict[str, Any]:
    if (
        ...
        raise QualificationError("refinement trace must be a bounded ordinary file")
    try:
        trace = object_(json.loads(path.read_bytes()), TRACE_FIELDS, "refinement trace")
    except (UnicodeDecodeError, json.JSONDecodeError, ValueError) as error:
        raise QualificationError("refinement trace must contain frozen JSON") from error
    if trace["schema"] != TRACE_SCHEMA:
        raise QualificationError("refinement trace schema differs")
    return trace
...
def assemble(
    ...
    *,
    ...
    parent = artifacts.get(parent_artifact_id)
    if parent is None or parent[1] != "model-bundle":
        raise QualificationError("refinement parent is absent from candidate")
    skeleton = {"run_id": run_id, "parent_artifact_id": parent_artifact_id}
    ...
    stage.mkdir()
    retained = stage / "refinement-execution.json"
    shutil.copyfile(trace_path, retained)
    anchor = derived["children"][-1]["artifact"]
