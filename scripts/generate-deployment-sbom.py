...
def _component_ref(name: str) -> str:
    return f"chart-file:{hashlib.sha256(name.encode('utf-8')).hexdigest()}"
    ...
    components = [
        {
            ...
            "properties": [
                {"name": "tritium:chart:member-bytes", "value": str(record["bytes"])},
                {"name": "tritium:chart:member-blake3", "value": record["blake3"]},
                {
                    "name": "tritium:chart:member-package-id",
