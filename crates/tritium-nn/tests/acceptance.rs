...
/// DRAFTER flash-attention profile gate (`set_m1_flash_attention`): under
/// the flash profile, `decode_greedy_chain` must still produce token-for-
/// token what per-step `decode_greedy_step` produces (both ride the same
/// captured graph — the chain only moves the feedback loop on-device), and
/// the whole path must serve a multi-chain schedule without error. Also
/// REPORTS flash-vs-exact first-chain agreement (no assert — the profile is
/// documented as not bit-identical to the exact pair; proposals-only).
#[cfg(feature = "cuda")]
#[test]
fn cuda_draft_chain_flash_matches_per_step() {
    let _gpu = gpu_serial();
    if !std::path::Path::new(&*DRAFTER_PATH).exists() {
        eprintln!("skipping: {} absent (drafter-gated test)", *DRAFTER_PATH);
        return;
    ...
    let Some(mut r_exact) = load_on("cuda", &bytes) else {
        return;
    };
    r_step.set_m1_flash_attention(true);
    r_chain.set_m1_flash_attention(true);

    let prompt: Vec<u32> = (1u32..=32).collect();
    let positions: Vec<usize> = (0..prompt.len()).collect();
    let l0 = r_step.forward(&prompt, &positions).expect("prefill A");
    let _ = r_chain.forward(&prompt, &positions).expect("prefill B");
    let _ = r_exact.forward(&prompt, &positions).expect("prefill C");
    let mut tok = argmax(&l0) as u32;
    let never_eos = u32::MAX;

    let mut pos = prompt.len();
    let mut first_chain: Option<Vec<u32>> = None;
    for &k in &[1usize, 4, 7, 16] {
        let mut per_step = Vec::with_capacity(k);
        let mut t = tok;
        for i in 0..k {
        ...
        let chained = r_chain
            .decode_greedy_chain(tok, pos, k, never_eos)
            .expect("flash chain ok")
            .expect("resident");
        assert_eq!(
            chained, per_step,
            "flash-profile chain(k={k}) diverged from per-step at pos {pos}"
        );
        if first_chain.is_none() {
            first_chain = Some(chained.clone());
        }
        pos += k;
        tok = *per_step.last().expect("k >= 1");
    }

    // Report (not gate): exact-profile agreement on the first chain.
    let exact = r_exact
        .decode_greedy_chain(argmax(&l0) as u32, prompt.len(), 1, never_eos)
        .expect("exact chain ok")
        .expect("resident");
    println!(
        "flash profile: chain==per-step over k=1/4/7/16; first flash draft {:?} vs exact {:?} ({})",
        first_chain.as_ref().and_then(|c| c.first()),
        exact.first(),
        if first_chain.as_ref().and_then(|c| c.first()) == exact.first() {
            "agree"
        } else {
            "diverge (documented: flash is not bit-identical to exact)"
        }
    );
}
