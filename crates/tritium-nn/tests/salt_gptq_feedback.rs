...
//!
//! This is the `fit_with_feedback` harness with its calibration-size, fold-size and propagation-decay
//! knobs. `salt_gptq.rs` beside it is the September research suite (activation-metric fit, discrete
//! trit search, asymmetric calibration) that grew from the same base on the experiment branch; the
//! two were merged as separate binaries rather than interleaved.
//!
...
//!   --test salt_gptq_feedback -- --ignored --nocapture
//! ```
