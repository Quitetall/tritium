...
//!
//! ## Rev 5: XOR-swizzled A tile (CODEGEN_REV 5)
//!
//! The rev-4 A-tile layout has shared-memory bank conflicts on the u32
//! fragment loads: a warp's 8 `group` lanes read the same 16-byte chunk of 8
//! different rows, and with `TILE_K/4` words per row the rows collide on the
//! same bank quads (4-way at `tile_k=64`, 2-way at `tile_k=32`, 8-way at
//! `tile_k=128`). Rev 5 XOR-swizzles the chunk index with a function of the
//! row — chunk' = chunk ^ ((row >> SW_SHIFT) & SW_MASK), with `SW_SHIFT`
//! chosen per chunks-per-row (`CH = tile_k/16`: CH>=8 → 0, CH=4 → 1, CH=2 → 2)
//! so the 8 group rows of one fragment load land on 8 distinct bank quads.
//! The swizzle is applied identically on the staging store (both the 16-byte
//! `cp.async` path and the byte fallback) and the fragment load, so it is a
//! pure **byte relayout** of shared memory: every value read is the value
//! staged, the mma operand order is untouched, and outputs stay bit-identical
//! to rev 4 (measured 1.26–1.28× on the tile_k=64 winners, neutral on
//! 128×128×32). B tiles stay unswizzled — their byte-granularity fragment
//! reads hit distinct words per group and broadcast within a group (verified
//! conflict-free).
//!
...
pub(crate) fn render_imma_source(cfg: TileConfig) -> String {
    ...
    // Rev-5 A-tile XOR swizzle (see module doc): CH 16-byte chunks per row;
    // chunk' = chunk ^ ((row >> SW_SHIFT) & (CH-1)). SW_SHIFT spreads the 8
    // `group` rows of one fragment load over distinct bank quads for each CH.
    // CH == 1 gives mask 0 (identity — that layout is already conflict-free).
    let sw_ch = (cfg.tile_k / 16) as u32;
    let sw_shift = match sw_ch {
        ch if ch >= 8 => 0u32,
        4 => 1,
        2 => 2,
        _ => 0,
    };
    let sw_mask = sw_ch.saturating_sub(1);

    format!(
        r#"// AUTO-GENERATED by tritium-cuda::codegen (WF-B nvrtc JIT, rev 5). Do not edit.
//
...
#define SW_SHIFT {sw_shift}
#define SW_MASK {sw_mask}

// Rev-5 A-tile chunk swizzle: a pure byte relayout of shared memory (store and
// load use the same mapping), so numerics are untouched. SW_MASK == 0 is the
// identity (tile_k == 16).
#define A_CHUNK_SW(row, chunk) ((chunk) ^ (((row) >> SW_SHIFT) & SW_MASK))
    ...
    auto stage = [&](int step, int buf) {{
        ...
        if (k16) {{
            ...
            for (int c = threadIdx.x; c < TILE_M * TILE_K / 16; c += blockDim.x) {{
                ...
                cp16(&s_a[buf][r * TILE_K + A_CHUNK_SW(r, kc) * 16], src, bytes);
            }}
        }} else {{
            ...
            for (int idx = threadIdx.x; idx < TILE_M * TILE_K; idx += blockDim.x) {{
                ...
                s_a[buf][r * TILE_K + A_CHUNK_SW(r, c >> 4) * 16 + (c & 15)] =
        ...
        if (computes) {{
            ...
            for (int kk = 0; kk < KTILES_PER_STEP; ++kk) {{
                ...
                for (int i = 0; i < WM_PER; ++i) {{
                    ...
                    // Swizzled u32 fragment loads: byte col (kbase{{+16}} + 4*tig)
                    // maps to chunk col>>4 (tig never crosses a chunk), relaid
                    // to A_CHUNK_SW(row, chunk) — the same mapping stage() wrote.
                    const int r0 = mm * IMMA_M + group;
                    const int r1 = mm * IMMA_M + group + 8;
                    const int col0 = kbase + 4 * tig;
                    const int col1 = kbase + 16 + 4 * tig;
                    const unsigned a0 = *(const unsigned*)(s_a[buf] + r0 * TILE_K
                        + A_CHUNK_SW(r0, col0 >> 4) * 16 + (col0 & 15));
                    const unsigned a1 = *(const unsigned*)(s_a[buf] + r1 * TILE_K
                        + A_CHUNK_SW(r1, col0 >> 4) * 16 + (col0 & 15));
                    const unsigned a2 = *(const unsigned*)(s_a[buf] + r0 * TILE_K
                        + A_CHUNK_SW(r0, col1 >> 4) * 16 + (col1 & 15));
                    const unsigned a3 = *(const unsigned*)(s_a[buf] + r1 * TILE_K
                        + A_CHUNK_SW(r1, col1 >> 4) * 16 + (col1 & 15));
                    #pragma unroll
                    for (int j = 0; j < WN_PER; ++j) {{
...
mod tests {
    ...
    fn rendered_source_has_kernel_symbol() {
        ...
        // Rev 5: the A-tile swizzle must be applied on BOTH the store and the
        // load side (a one-sided swizzle would scramble the operands).
        assert!(
            src.matches("A_CHUNK_SW(").count() >= 7,
            "expected the A-tile swizzle at the defines + both staging paths + \
             the four fragment loads"
        );
        assert!(
