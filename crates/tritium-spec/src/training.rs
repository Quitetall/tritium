...
/// Frozen descriptor shared by versioned training-operation manifests.
...
static OPERATIONS_V1: &[TrainingOpDescriptorV1] = &[
...
static OPERATIONS_V2: &[TrainingOpDescriptorV1] = &[
    graph("graph.ste_surrogate"),
    graph("graph.salt_ste"),
    graph("graph.lsq_ste"),
    graph("graph.fsq"),
...
/// Strict training-operation manifest parse failure.
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::OperationMismatch { index, id } => write!(
                f,
                "training manifest operation {index} ({id:?}) differs from frozen descriptor"
            ),
        }
    }
}
...
impl TrainingOpManifestV1 {
    ...
    pub fn parse_json(bytes: &[u8]) -> Result<Self, TrainingManifestError> {
        validate_manifest(bytes, Self::SCHEMA_VERSION, OPERATIONS_V1)?;
        Ok(Self)
    }
}

/// Frozen language-neutral v2 portable-training registry.
#[derive(Clone, Copy, Debug, Default, Eq, PartialEq)]
pub struct TrainingOpManifestV2;

impl TrainingOpManifestV2 {
    /// Permanent schema identifier.
    pub const SCHEMA_ID: &'static str = SCHEMA_ID;
    /// Frozen schema version.
    ...
    /// Parse and validate a manifest against complete frozen v2 registry.
    ///
    /// # Errors
    /// Returns [`TrainingManifestError`] for malformed JSON or any schema,
    /// dtype, count, order, identifier, capability or state-plane mismatch.
...
mod tests {
    ...
    fn canonical_manifest_roundtrips_and_registry_is_unique() {
        assert_eq!(
            TrainingOpManifestV2::parse_json(CANONICAL_JSON_V2),
            Ok(TrainingOpManifestV2)
        );
        assert_eq!(CANONICAL_JSON_V2.last(), Some(&b'\n'));
        for (index, operation) in OPERATIONS_V2.iter().enumerate() {
    ...
    fn strict_parser_rejects_schema_fields_and_registry_drift() {
        let source = core::str::from_utf8(CANONICAL_JSON_V2).unwrap();
        let cases = [
        ...
        for case in cases {
            assert!(TrainingOpManifestV2::parse_json(case.as_bytes()).is_err());
        }
    }

    #[test]
    fn strict_parser_rejects_missing_duplicate_and_reordered_operations() {
        let mut value: serde_json::Value = serde_json::from_slice(CANONICAL_JSON_V2).unwrap();
        let operations = value["operations"].as_array_mut().unwrap();
        operations.pop();
        assert!(matches!(
            TrainingOpManifestV2::parse_json(serde_json::to_string(&value).unwrap().as_bytes()),
    ...
    #[test]
    fn v1_remains_exact_and_rejects_v2() {
        assert_eq!(
            TrainingOpManifestV1::parse_json(CANONICAL_JSON_V1),
            Ok(TrainingOpManifestV1)
        );
        assert_eq!(TrainingOpManifestV1::operations().len(), 35);
        assert_eq!(TrainingOpManifestV2::operations().len(), 36);
        assert!(TrainingOpManifestV1::parse_json(CANONICAL_JSON_V2).is_err());
        assert!(TrainingOpManifestV2::parse_json(CANONICAL_JSON_V1).is_err());
    }
}
