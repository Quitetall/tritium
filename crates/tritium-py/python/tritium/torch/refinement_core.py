...
def _dense_sweep(
    ...
    for plane_index, scale in enumerate(scales):
        other = (
            _decoded(
                tuple(
                    value for index, value in enumerate(result) if index != plane_index
                ),
                tuple(
                    value for index, value in enumerate(scales) if index != plane_index
                ),
                dtype,
            )
            if len(result) > 1
...
def _s34_sweep(
    ...
    for plane_index, scale in enumerate(scales):
        ...
        for index, pattern in enumerate(patterns):
            error = (
                (residual - row_scale * pattern.reshape(1, 1, 4)).square() * curvature
            ).sum(dim=2)
            accept = error < best_error
            best_error[accept] = error[accept]
            best_index[accept] = index
        result[plane_index] = patterns[best_index].reshape(rows, columns).to(torch.int8)
    return tuple(result)
