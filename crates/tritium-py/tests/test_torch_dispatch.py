...
def test_ternary_linear_supports_functorch_grad_and_vmap():
    ...
    direct_no_bias = torch.func.vmap(
        lambda sample, master: torch.ops.tritium.ternary_linear.default(
            sample, master
        )
    )(vector_samples, batched_weight)
    no_bias_expected = torch.stack(
        [reference_ternary_linear(x_i, w_i) for x_i, w_i in zip(
            vector_samples, batched_weight
        )]
    )
    torch.testing.assert_close(direct_no_bias, no_bias_expected)
