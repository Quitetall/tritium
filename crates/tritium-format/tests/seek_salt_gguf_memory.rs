...
use tritium_format::{SaltGgufReader, SaltRow, TQ2_0_BLOCK_BYTES, num_blocks, write_salt_gguf};
...
#[test]
fn strict_reader_allocation_is_row_bounded_not_artifact_bounded() {
    let k = 1_000_000;
    ...
    let artifact = write_salt_gguf(&[
        ("large.unselected", large_rows.as_slice()),
        ("small.selected", small_rows.as_slice()),
    ])
    .unwrap();
    drop(large_rows);
    assert!(artifact.len() > 16 * 1024 * 1024);

    ALLOCATED_BYTES.store(0, Ordering::Relaxed);
    TRACK_ALLOCATIONS.store(true, Ordering::SeqCst);
    let mut reader = SaltGgufReader::new_strict(Cursor::new(artifact.as_slice())).unwrap();
    reader
    ...
    assert!(
        ...
        "strict scan allocated {allocated} bytes for a {}-byte borrowed artifact",
        artifact.len()
    );
}
